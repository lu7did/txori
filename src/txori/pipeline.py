...
class Pipeline:
    ...
    def __post_init__(self) -> None:
        ...
        # Constantes para calibrar ruido en dominio espectral (n_fft=2048, Kaiser beta=14)
        self._noise_cal_nfft = 2048
        self._noise_cal_beta = 14.0
        import numpy as _np
        _w = _np.kaiser(self._noise_cal_nfft, self._noise_cal_beta)
        self._noise_wsum = float(_w.sum())
        self._noise_wsum2 = float((_w * _w).sum())
        # Factor medio de magnitud de ruido (Rayleigh) por bin: E|N_k| = sigma * sqrt(pi/2) * sqrt(sum(w^2)/2)
        self._noise_rayleigh_k = float((_np.pi / 2.0) ** 0.5 * (_np.sqrt(self._noise_wsum2 / 2.0)))
