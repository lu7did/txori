...
def test_filesource_stereo_mixes_to_mono() -> None:
    with TemporaryDirectory() as td:
        sr = 8000
        t = np.arange(sr) / sr
        left = (np.sin(2 * np.pi * 440 * t) * 0.9 + 0.0)
        right = (np.sin(2 * np.pi * 880 * t) * 0.9 + 0.0)
        d16_left = np.clip((left * 32767).astype(np.int16), -32768, 32767)
        d16_right = np.clip((right * 32767).astype(np.int16), -32768, 32767)
        wav = os.path.join(td, "stereo.wav")
        _write_wav_stereo(wav, sr, 2, d16_left, d16_right)
        src = FileSource(wav)
        y = src.read(2000)
